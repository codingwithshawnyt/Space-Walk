...
/* This script is used to make the camera to follow the player’s movements. The Update method ensures that the camera’s position is updated every frame to match the player’s position. 
The [SerializeField] attribute is used to allow the player variable to be set directly from the Unity editor, providing a convenient way to link this script to the player object. 
The Transform class represents an object’s position, rotation and scale in 3D space. In this case, it’s used to track and follow the player’s position. 
The Vector3 class represents 3D vectors and points, used here to create a new position for the camera based on the player’s position. The MonoBehaviour class is a base class every script you create in Unity inherits from. 
It provides life-cycle functions such as Update(). This script should be attached to the camera object in Unity. */
